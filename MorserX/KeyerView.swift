...
class Keyer: ObservableObject {
    ...
    /// The library owns the table. This used to be a 50-entry copy of it, which
    /// is a second source of truth for facts that only have one source.
    private func morseToLetter(_ m: String) -> String? {
        Morse.character(for: m).map(String.init)
    }
